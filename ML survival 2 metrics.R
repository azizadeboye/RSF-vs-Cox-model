...
set.seed(123)
mydata <- tbsurv
# mydata <- tbsurv[, -1] to remove any unwanted column
...
P1 = ggplot(data_Cindex, aes(x = model, y = Cindex, fill = model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#FFBBCC", "#88CCFF"))

P2 = ggplot(data_BS, aes(x = model, y = BS, fill = model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#FFBBCC", "#88CCFF")) 

P3 = ggplot(data_IBS, aes(x = model, y = IBS, fill = model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#FFBBCC", "#88CCFF")) 

P4 = ggplot(data_IAE, aes(x = model, y = IAE, fill = model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#FFBBCC", "#88CCFF")) 

P5 = ggplot(data_ISE, aes(x = model, y = ISE, fill = model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#FFBBCC", "#88CCFF")) 
...
#plot
data_Cindex = data.frame('Cindex' = c(Cindex_cox, Cindex_rsf, Cindex_rsf_cox),
                         'model' = c(rep('Cox', 10), rep('RSF', 10), rep('Rsf-Cox', 10)))

data_BS = data.frame('BS' = c(BS_cox, BS_rsf, BS_rsf_cox),
                     'model' = c(rep('Cox', 10), rep('RSF', 10), rep('Rsf-Cox', 10)))

data_IBS = data.frame('IBS' = c(IBS_cox, IBS_rsf, IBS_rsf_cox),
                      'model' = c(rep('Cox', 10), rep('RSF', 10), rep('Rsf-Cox', 10)))

data_IAE = data.frame('IAE' = c(IAE_cox, IAE_rsf, IAE_rsf_cox),
                      'model' = c(rep('Cox', 10), rep('RSF', 10), rep('Rsf-Cox', 10)))

data_ISE = data.frame('ISE' = c(ISE_cox, ISE_rsf, ISE_rsf_cox),
                      'model' = c(rep('Cox', 10), rep('RSF', 10), rep('Rsf-Cox', 10)))
